...
export class ContactService implements IContactService {
  ...
  async createContact(payload: CreateContactRequestDTO) {
    await this._contactRepository.create({
      ...payload,
      userId: payload.userId ? toObjectId(payload.userId) : null,
    });
  }

  async contactEnquiries(
    page: number,
    limit: number,
    search: string,
    status: ContactStatus,
  ): Promise<ContactEnquiriesResponseDTO> {
    const { enquiries, totalCount, pendingCount, resolvedCount } =
      await this._contactRepository.findAllEnquiries(page, limit, search, status);
  ...
  async updateEnquiry(enquiryId: string): Promise<string> {
    ...
    if (enquiry.status === CONTACT_STATUS.RESOLVED)
      throw new AppError(ERROR_MESSAGES.ENQUIRY_ALREADY_RESOLVED, HTTP_STATUS.BAD_REQUEST);

    const updatedEnquiry = await this._contactRepository.findOneAndUpdate(
      { _id: toObjectId(enquiryId) },
