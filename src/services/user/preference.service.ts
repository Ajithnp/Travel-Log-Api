...
import {
  FetchUserPreferencesResult,
  IUserPreferenceService,
} from '../../interfaces/service_interfaces/user/IPreferenceService';
...
import {
  NEW_USER_PACKAGES_RECOMMENDATION_PROMPT,
  PREFERENCE_SUMMARY_PROMPT,
  PREFERENCE_SUMMARY_SYSTEM_PROMPT,
} from '../../shared/templates/prompt_templates';
...
export class UserPreferenceService implements IUserPreferenceService {
  ...
  async generatePreferenceSummary(profile: PreferenceProfile): Promise<string> {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      ['system', PREFERENCE_SUMMARY_SYSTEM_PROMPT],
      ['human', PREFERENCE_SUMMARY_PROMPT],
    ]);
