...
import {
  IPayoutService,
  PayoutOverviewResponseDto,
  PayoutScheduleListResponseDto,
  PayoutStatsResponseDto,
  ReleasePayoutResponseDTO,
  FindAllPayoutsResponseDto,
  SchedulePayoutDetailsResponseDTO,
...
export class PayoutService implements IPayoutService {
  ...
  async getPayoutSchedules(
    ...
    limit: number,
    search: string,
  ): Promise<PaginatedData<PayoutScheduleListResponseDto>> {
  ...
  async releasePayout(scheduleId: string): Promise<ReleasePayoutResponseDTO> {
    ...
    if (!vendorInfo?.transactionConnect?.payoutsEnabled) {
      throw new AppError(ERROR_MESSAGES.VENDOR_PAYOUT_NOT_ENABLED, HTTP_STATUS.BAD_REQUEST);
    }
  ...
  async findAllPayouts(
    ...
    search?: string,
    filter?: PayoutFilter,
  ): Promise<PaginatedData<FindAllPayoutsResponseDto>> {
  ...
  async findAllVendorPayouts(
    vendorId: string,
    page: number,
    limit: number,
    search?: string,
    filter?: PayoutFilter,
  ): Promise<PaginatedData<VendorPayoutsListResponseDto>> {
    const data = await this._payoutRepository.findAllPayoutsByVendor(
      vendorId,
      page,
      limit,
      search,
