...
import {
  ...
  DashboardTopPerformersDto,
  IAdminService,
  PlatformRevanueTrendResponseDto,
  TrendChartDataPoint,
} from 'interfaces/service_interfaces/admin/IAdminService';
...
export class AdminService implements IAdminService {
  ...
  async dashboardActionsRequired(): Promise<DashboardActionsRequiredResponseDto> {
    const [pendingVendorVerifications, pendingCancellationRequests, pendingPayouts, failedPayouts] =
      await Promise.all([
  ...
  async dashboardRevenueTrend(
    period: string,
    customFrom?: string,
    customTo?: string,
  ): Promise<PlatformRevanueTrendResponseDto> {
