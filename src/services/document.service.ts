...
export class DocumentService implements IDocumentService {
    ...
    async getBookingTicket(bookingId: string, userId: string): Promise<{ buffer: Buffer; filename: string }> {
    ...
    private buildBookingTicketPDF(data: TicketData, qrBuffer: Buffer): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            ...
            doc
                .fillColor(TEXT2)
                .fontSize(9)
                .text(`Operated by: ${data.vendorName}`, pageLeft, y)

            y = doc.y + 16


            doc.moveTo(pageLeft, y).lineTo(pageLeft + pageWidth, y).strokeColor(MUTED).lineWidth(1).stroke()
            y += 16

            const col1 = pageLeft
            const col2 = pageLeft + pageWidth / 2 + 10

            const drawField = (label: string, value: string, x: number, currentY: number, width = 200) => {
                doc
                    .fillColor(TEXT2)
                    .font('Helvetica')
                    .fontSize(8)
            ...
            doc
                ...
                .font('Helvetica-Bold')
                .fontSize(11)
                .text('Payment Summary', pageLeft, y)
            ...
            doc
                ...
                .text(
                    ...
                    0,
                    doc.page.height - 27,
                    { width: doc.page.width - 48, align: 'right', lineBreak: false }
                )
