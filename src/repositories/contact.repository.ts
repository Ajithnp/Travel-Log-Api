...
async findAllEnquiries(page: number, limit: number, search: string, status: ContactStatus):Promise<{enquiries:Partial<IContact>[],totalCount:number,pendingCount:number,resolvedCount:number}>{
        ...
        { $match: filter },
        {
            $facet: {
                enquiries: [
                    ...
                    { $skip: skip },
                    { $limit: limit },
                    { $project: { name: 1, email: 1, phone: 1, subject: 1, message: 1, isGuest: 1, status: 1, createdAt: 1, updatedAt: 1 } }
                ],
