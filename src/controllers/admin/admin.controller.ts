...
export class AdminController implements IAdminController {
  ...
  dashboardTopPerformers = expressAsyncHandler(
  ...
  dashboardActionsRequired = expressAsyncHandler(
    async (req: Request, res: Response): Promise<void> => {
  ...
  dashboardRevenueTrend = expressAsyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const { period, customFrom, customTo } = req.query as {
        period: string;
        customFrom?: string;
        customTo?: string;
      };
      const result = await this._adminService.dashboardRevenueTrend(
        period || 'week',
        customFrom,
        customTo,
      );
