...
export class BookingMapper {
    ...
    static toDetailedResponse(booking: RawPopulatedBooking):  BookingDetailDTO  {
        ...
        const itinerary: ItineraryDayDTO[] = (pkg?.itinerary ?? [])
            .slice()
            .sort((a: RawItineraryDay, b: RawItineraryDay) =>
                (a.dayNumber ?? 0) - (b.dayNumber ?? 0)
            )
            .map((day: RawItineraryDay): ItineraryDayDTO => ({
                dayNumber: day.dayNumber ?? 0,
                title: day.title ?? null,
                activities: (day.activities ?? []).map((act: RawActivity): ActivityDTO => ({
  ...
  const vendor = booking.vendorId
        ...
        const travelers: TravelerDTO[] = (booking.travelers ?? [])
            .slice()
            .sort((a: RawTraveler, b: RawTraveler) =>
                (b.isLead ? 1 : 0) - (a.isLead ? 1 : 0)
            )
            .map((t: RawTraveler): TravelerDTO => ({
                fullName: t.fullName ?? '',
                idType: t.idType ?? '',
                idNumber: t.idNumber ?? '',
