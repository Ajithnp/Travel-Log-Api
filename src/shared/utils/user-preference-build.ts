...
export function buildPreferenceProfile(
  ...
  const bookedPackageIds = bookings
    .map((b) => b.packageId?._id?.toString())
  ...
  const travelStyle = groupTypes.length
    ? groupTypes
        .sort(
          (a, b) =>
            groupTypes.filter((v) => v === b).length - groupTypes.filter((v) => v === a).length,
        )
        .at(0) || null
    : null;

  const bookedLocations = bookings.map((b) => b.packageId?.location).filter(Boolean) as string[];

  const bookedStates = bookings.map((b) => b.packageId?.state).filter(Boolean) as string[];
  ...
  const wishlistLocations = wishlistPackages.map((p) => p.location).filter(Boolean) as string[];

  const wishlistStates = wishlistPackages.map((p) => p.state).filter(Boolean) as string[];
